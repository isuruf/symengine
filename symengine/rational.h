...
public:
    ...
    * */
    ...
    bool is_canonical(const rational_class &i);
    ...
    * */
    ...
    inline RCP<const Number> powrat(const Integer &other) const {
        ...
        integer_class num;
        mpz_pow_ui(get_mpz_t(num), get_mpz_t(get_num(this->i)), exp);

        integer_class den;
        mpz_pow_ui(get_mpz_t(den), get_mpz_t(get_den(this->i)), exp);

        // Since 'this' is in canonical form, so is this**other, so we simply
        // pass num/den into the constructor directly:
        if (not neg)
            if (abs(den) == one->i)
                return integer(std::move(num*sign(den)));
            else
                return make_rcp<const Rational>(rational_class(std::move(num*sign(den)), std::move(abs(den))));
        else
            if (abs(num) == one->i)
                return integer(std::move(den*sign(num)));
            else
                return make_rcp<const Rational>(rational_class(std::move(den*sign(num)), std::move(abs(num))));
    }
