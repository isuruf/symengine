...
RCP<const UnivariatePolynomial> UnivariatePolynomial::from_vec(const RCP<const Symbol> &var, const std::vector<integer_class> &v){
    map_uint_mpz dict;
    unsigned int degree = 0;
...
RCP<const UnivariatePolynomial> mul_uni_poly(RCP<const UnivariatePolynomial> a, RCP<const UnivariatePolynomial> b) {
    ...
    if (sign == -1)
        return neg_uni_poly(*UnivariatePolynomial::from_vec(a->var_, v));
    else
        return UnivariatePolynomial::from_vec(a->var_, v);
}
